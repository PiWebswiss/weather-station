                ...
                with self.condition:
                    ...
                    if refresh_action:
                        ...
                        if image_hash != latest_refresh.image_hash:
                            ...
                            use_partial = partial_zone and partial_zone != "all"
                            if use_partial:
                                logger.info(f"Partial refresh for zone '{partial_zone}'. | refresh_info: {refresh_info}")
                                self.display_manager.display_partial_image(image, image_settings=plugin.config.get("image_settings", []))
                            else:
                                logger.info(f"Updating display (full refresh). | refresh_info: {refresh_info}")
...
class RefreshAction:
    ...
    def get_plugin_settings(self):
        """Return the plugin settings dict for this refresh action, or None."""
        return None
...
class ManualRefresh(RefreshAction):
    ...
    def get_plugin_settings(self):
        """Return the plugin settings for this manual refresh."""
...
class PlaylistRefresh(RefreshAction):
    ...
    def get_plugin_settings(self):
        """Return the plugin instance settings (contains partialZone if configured)."""
